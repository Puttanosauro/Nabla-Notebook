package org.example.project.qdcore.lexer.tokens

import org.example.project.qdcore.lexer.Token
import org.example.project.qdcore.lexer.TokenData
import org.example.project.qdcore.visitor.token.TokenVisitor

/**
 * A blank line.
 * @see org.example.project.qdcore.ast.base.block.Newline
 */
class NewlineToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 *     Code
 * ```
 * @see org.example.project.qdcore.ast.base.block.Code
 */
class BlockCodeToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Examples:
 * ~~~
 * ```language
 * Code
 * ```
 * ~~~
 *
 * ```
 * ~~~language
 * Code
 * ~~~
 * ```
 * @see org.example.project.qdcore.ast.base.block.Code
 */
class FencesCodeToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * A multiline fenced block that contains a TeX expression.
 * This is a custom Quarkdown block.
 *
 * Example:
 * $$$
 * LaTeX expression line 1
 * LaTeX expression line 2
 * $$$
 * @see org.example.project.qdcore.ast.quarkdown.block.Math
 */
class MultilineMathToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * An isolated one-line fenced block that contains a TeX expression.
 * If it's not isolated, then it's an [InlineMathToken].
 * This is a custom Quarkdown block.
 *
 * Example:
 * $ LaTeX expression $
 * @see org.example.project.qdcore.ast.quarkdown.block.Math
 */
class OnelineMathToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * A thematic break.
 * Examples:
 * ```
 * ---
 * ```
 * ```
 * *****
 * ```
 * @see org.example.project.qdcore.ast.base.block.HorizontalRule
 */
class HorizontalRuleToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * A page break.
 * This is a custom Quarkdown block.
 *
 * Example:
 * ```
 * <<<
 * ```
 * @see org.example.project.qdcore.ast.quarkdown.block.PageBreak
 */
class PageBreakToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * # Heading
 * ```
 * @see org.example.project.qdcore.ast.base.block.Heading
 */
class HeadingToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Examples:
 * ```
 * Heading
 * ====
 * ```
 * ```
 * Heading
 * ---
 * ```
 * @see org.example.project.qdcore.ast.base.block.Heading
 */
class SetextHeadingToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * [label]: url "Title"
 * ```
 * @see org.example.project.qdcore.ast.base.block.LinkDefinition
 */
class LinkDefinitionToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * [^label]: Lorem ipsum
 *   dolor sit
 * amet.
 * ```
 * @see org.example.project.qdcore.ast.base.block.FootnoteDefinition
 */
class FootnoteDefinitionToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * - A
 * - B
 * ```
 * @see org.example.project.qdcore.ast.base.block.list.UnorderedList
 */
class UnorderedListToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * 1. First
 * 2. Second
 * ```
 * @see org.example.project.qdcore.ast.base.block.list.OrderedList
 */
class OrderedListToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Examples:
 * ```
 * - A
 * ```
 * ```
 * 1. First
 * ```
 * @see org.example.project.qdcore.ast.base.block.list.ListItem
 */
class ListItemToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Examples:
 * ```
 * | foo | bar |
 * | --- | --- |
 * | baz | bim |
 * ```
 * ```
 * | foo  |  bar |
 * | :--- | ---: |
 * | baz  |  bim |
 * ```
 * @see org.example.project.qdcore.ast.base.block.Table
 */
class TableToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * <p>
 *     Content
 * </p>
 * ```
 * @see org.example.project.qdcore.ast.base.block.Html
 */
class HtmlToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * @see org.example.project.qdcore.ast.base.block.Paragraph
 */
class ParagraphToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * Example:
 * ```
 * > Quote
 * ```
 * @see org.example.project.qdcore.ast.base.block.BlockQuote
 */
class BlockQuoteToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}

/**
 * @see org.example.project.qdcore.ast.base.block.BlankNode
 */
class BlockTextToken(
    data: TokenData,
) : Token(data) {
    override fun <T> accept(visitor: TokenVisitor<T>) = visitor.visit(this)
}
